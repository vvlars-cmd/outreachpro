...
// ââ Ensure data dir exists ââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
// ââ Seed data (first run) âââââââââââââââââââââââââââââââââââââââââââââââââââââ
function seedIfEmpty(file, data) {
  const p = path.join(DATA_DIR, file);
  if (!fs.existsSync(p)) fs.writeFileSync(p, JSON.stringify(data, null, 2));
}

// Seed leads â read from file if it exists, else use inline defaults
...
const leadsSeed = fs.existsSync(leadsFile)
  ...
  : [
    { id:'l1', name:'Fahrrad Jungmann', city:'Wilhelmshaven', email:'info@fahrrad-jungmann.de', category:'FahrradhÃ¤ndler', rating:4.5, reviewsCount:38, status:'Neu', src:'CRM', website:'fahrrad-jungmann.de' },
    { id:'l2', name:'Rad Sport Weber', city:'BrÃ¼hl', email:'info@radsport-weber.de', category:'FahrradhÃ¤ndler', rating:4.2, reviewsCount:55, status:'Neu', src:'CRM', website:'radsport-weber.de' },
    { id:'l3', name:'Bike World MÃ¼nster', city:'MÃ¼nster', email:'info@bikeworld-ms.de', category:'FahrradhÃ¤ndler', rating:4.7, reviewsCount:112, status:'Kontaktiert', src:'CRM', website:'bikeworld-ms.de' },
    { id:'l4', name:'E-Bike Center KÃ¶ln', city:'KÃ¶ln', email:'info@ebike-center.de', category:'E-Bike HÃ¤ndler', rating:4.4, reviewsCount:89, status:'Neu', src:'CRM', website:'ebike-center.de' },
    { id:'l5', name:'Zweirad Hoffmann', city:'Aachen', email:'info@zweirad-hoffmann.de', category:'FahrradhÃ¤ndler', rating:4.6, reviewsCount:74, status:'Qualifiziert', src:'CRM', website:'zweirad-hoffmann.de' },
  ];
seedIfEmpty('leads.json', leadsSeed);
seedIfEmpty('campaigns.json', [
  { id:'c1', name:'CycleWASH Spring 2026 â NRW', status:'active', from:'info@cyclewash.de', daily:40, stopOnReply:true, leads:25, sent:18, opens:11, replies:3, bounced:0, openRate:61, replyRate:17, createdAt:'2026-03-15T09:00:00Z', subject:'CycleWASH Pro Platinum â Vorstellung fÃ¼r {{name}}' },
  { id:'c2', name:'Events Follow-up â Jever & BrÃ¼hl', status:'paused', from:'info@cyclewash.de', daily:25, stopOnReply:true, leads:8, sent:8, opens:5, replies:2, bounced:0, openRate:63, replyRate:25, createdAt:'2026-03-10T10:30:00Z', subject:'Nachfassung: CycleWASH Demo in {{city}}' },
]);
...
// Seed events â read from file if it exists, else use inline defaults
...
const eventsSeed = fs.existsSync(eventsFile)
  ...
  : [
    ...
    { id:'e2', name:'Fahrradmesse BrÃ¼hl', city:'BrÃ¼hl', date:'2026-05-03', leads:4 },
    { id:'e3', name:'E-Bike Expo DÃ¼sseldorf', city:'DÃ¼sseldorf', date:'2026-05-17', leads:6 },
    { id:'e4', name:'Radmarkt Aachen', city:'Aachen', date:'2026-06-07', leads:3 },
    { id:'e5', name:'Bike Days MÃ¼nster', city:'MÃ¼nster', date:'2026-06-21', leads:5 },
  ];
seedIfEmpty('events.json', eventsSeed);

// Seed deals, inbox, sequences
seedIfEmpty('deals.json', {
  Prospect:[{id:'deal1',company:'Rad & Tat KÃ¶ln',value:38000,contact:'Michael Brandt',email:'m.brandt@raduntat.de'}],
  Contacted:[{id:'deal2',company:'Bike Garage DÃ¼sseldorf',value:27500,contact:'Anna Fleischer',email:'info@bikegarage.de'}],
  Meeting:[], Proposal:[{id:'deal3',company:'Velo Center Bonn',value:76000,contact:'Petra Schmidt',email:'p.schmidt@velocenter.de'}],
  Won:[{id:'deal4',company:'Bike World MÃ¼nster',value:27500,contact:'Hans Weber',email:'h.weber@bikeworld.de'}], Lost:[]
});
seedIfEmpty('inbox.json', [
  {id:'msg1',from:'Karl Jungmann & Sohn',email:'info@fahrrad-jungmann.de',subject:'Re: CycleWASH Pro Platinum',preview:'Das klingt sehr interessant fÃ¼r uns...',body:'<p>Guten Tag,</p><p>vielen Dank! Das klingt sehr interessant. KÃ¶nnen Sie mir mehr Details schicken?</p><p>MfG, Karl Jungmann</p>',ts:'2026-03-26T14:23:00Z',label:'interested',read:false},
...
seedIfEmpty('sequences.json', {
  c1:[
    {id:'s1',subject:'CycleWASH Pro Platinum â Vorstellung fÃ¼r {{name}}',body:'<p>Guten Tag,</p><p>ich mÃ¶chte Ihnen die CycleWASH Pro Platinum vorstellen â eine vollautomatische Fahrradwaschanlage fÃ¼r FachhÃ¤ndler.</p><p>{{aiOpening}}</p><p>HÃ¤tten Sie 15 Minuten?</p><p>Mit freundlichen GrÃ¼Ãen,<br/>Sebastian MÃ¼ller<br/>CW Cleaning Solutions GmbH</p>',delay:0,variant:'A'},
    {id:'s2',subject:'Re: CycleWASH â kurze Nachfassung',body:'<p>Guten Tag,</p><p>kurze Nachfassung zu meiner E-Mail von letzter Woche. Darf ich Ihnen weitere Infos schicken?</p><p>Beste GrÃ¼Ãe,<br/>Sebastian</p>',delay:3,variant:'A'},
    {id:'s3',subject:'CycleWASH â Fallstudie: +22% Kundenzufriedenheit',body:'<p>Guten Tag,</p><p>als letzten Kontaktversuch teile ich eine Fallstudie: ein HÃ¤ndler steigerte die Kundenzufriedenheit um 22%.</p><p>Beste GrÃ¼Ãe,<br/>Sebastian</p>',delay:7,variant:'A'},
  ]
});
...
app.post('/api/apify/scrape', async (req, res) => {
  sseSetup(res);
  const { cities = [], searchTerms = ['FahrradgeschÃ¤ft','E-Bike HÃ¤ndler'], maxPerSearch = 15, minStars = 3.5 } = req.body;
...
app.post('/api/ai/personalise', async (req, res) => {
  ...
  const langPrompts = {
    de: `Schreibe einen personalisierten Einstiegssatz auf Deutsch fÃ¼r eine Kaltakquise-E-Mail an dieses Unternehmen. Nutze die Daten:`,
    en: `Write a personalised opening sentence in English for a cold outreach email to this business. Use the data:`,
    fr: `Ãcris une phrase d'accroche personnalisÃ©e en franÃ§ais pour un e-mail de prospection Ã  cette entreprise. Utilise les donnÃ©es:`,
    es: `Escribe una frase de apertura personalizada en espaÃ±ol para un correo de prospecciÃ³n a esta empresa. Usa los datos:`,
...
app.post('/api/ai/copilot', async (req, res) => {
  ...
  const system = `You are the OutreachPro AI Copilot â an expert B2B sales engagement assistant. You help users find leads, write email sequences, analyse campaign performance, and optimise outreach. ${systemContext}`.trim();
...
// ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
// GMAIL SEND (SSE â rate-limited queue)
// ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
// ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
// GMAIL SCRAPER (inbox â leads)  SSE
// ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
// ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
// LEAD IMPORT (CSV upload â multipart)
// ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
// ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
// AI BATCH PERSONALISE â SSE stream for all leads
// ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
app.post('/api/ai/personalise/batch', async (req, res) => {
  ...
  const langPrompts = {
    de: 'Schreibe einen personalisierten Einstiegssatz auf Deutsch fÃ¼r eine B2B Kaltakquise-E-Mail. Nur den Satz, keine BegrÃ¼Ãung.',
    en: 'Write a personalised opening sentence in English for a B2B cold outreach email. Only the sentence, no greeting.',
    fr: 'Ãcris une phrase d\'accroche personnalisÃ©e en franÃ§ais. Seulement la phrase.',
    es: 'Escribe una frase de apertura personalizada en espaÃ±ol. Solo la frase.',
...
// ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
// SPA FALLBACK (must be LAST â after all API routes)
// ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
app.get('*', (req, res) => {
